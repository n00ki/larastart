  ...
  <div class="mx-auto flex h-16 items-center px-4 md:max-w-7xl">
    ...
    <div class="lg:hidden">
      <Sheet.Root>
        ...
        <Sheet.Content side="left" class="flex h-full w-64 flex-col items-stretch justify-between bg-sidebar">
            ...
            <div class="flex h-full flex-col justify-between text-sm">
              <div class="flex flex-col space-y-4">
                {#each mainNavItems as item (item.title)}
                  <Link href={item.href} class="flex items-center space-x-2 font-medium">
                    {#if item.icon}
                      <Icon name={item.icon} />
                    {/if}
          ...
          {#each mainNavItems as item, index (index)}
            <NavigationMenu.Item class="relative flex h-full items-center">
              ...
              {#if $page.url === item.href}
                <div class="absolute bottom-0 left-0 h-0.5 w-full translate-y-px bg-foreground"></div>
              {/if}
            </NavigationMenu.Item>
          {/each}
        </NavigationMenu.List>
