<script lang="ts">
  ...
  let { tooltipPosition = "left", class: className = "", ...props }: Props = $props();
  ...
  const currentMode = $derived(modes.find(mode => mode.value === theme.current)!);
  const nextMode = $derived(modes[(modes.indexOf(currentMode) + 1) % modes.length]);
